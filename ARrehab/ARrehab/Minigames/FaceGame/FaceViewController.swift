...
class FaceViewController: UIViewController, ARSessionDelegate {
    ...
    ///  Judge whether an expression is correctly matched; update progress bar, points (of an expression),
    func processNewARFrame() {
        ...
        if let currentExpression = self.currentExpression, let shownAt = self.currentExpressionShownAt, let faceAnchor = self.currentFaceAnchor {
            ...
            // Get the number of points an expression corresponds to
    ...
    /// adjust stage (difficulty) based on performance, assign the next randomly-generated expression
...
//            self.saveImageToFaceDirectory(image)
    ...
    func failedCurrentExpression() {
        ...
        // shake animation for action label
        self.actionLabel.doIncorrectAttemptShakeAnimation()
        self.livesLeft -= 1
        
        if self.livesLeft == 0 {
            // when no lives left, end the game
            self.currentExpression = nil
            self.showFaceImages()
...
//    }
    ...
    func updateUI() {
        if gameActive {
            // update action label with current expression name when game is active
            if let expression = self.currentExpression {
                self.actionLabel.text = expression.name()
            }
            // show action label, timeleft bar, points left when game is active
            self.actionLabel.isHidden = false
            self.startGameButton.isHidden = true
            self.timeLeftBar.isHidden = false
            self.pointsLeftLabel.isHidden = false
        } else {
            // show "Start Game" button when game is not active
